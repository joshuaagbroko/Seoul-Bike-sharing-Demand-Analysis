...
get_weather_forecaset_by_cities <- function(city_names){
  ...
  for (city_name in city_names){
    ...
    api_key <- "9c83fde3f1cb401c95403fdaebf49055"
...
generate_city_weather_bike_data <- function (){
  ...
  cities_bike_pred <- cities_df %>% left_join(results) %>% 
    dplyr::select(CITY_ASCII, LNG, LAT, TEMPERATURE, HUMIDITY, BIKE_PREDICTION, BIKE_PREDICTION_LEVEL, LABEL, DETAILED_LABEL, FORECASTDATETIME)
